...
const RESPONSE_HELPER = require('./response-helper');
...
function Router({
    ...
    const route = async (event, context, config) => {
        ...
        try {
            ...
            console.log(config, "Route error", { error: error });
        ...
        // Route
        try {
            console.log(config, "Route handler processing", { requestPath, httpMethod });
            let result;
            if(route){
                let verifyUser =await( tokenVerifier.verifyToken(event, context, config));
                result = await (route.handler(event, context, config));
            }else{
               result = await (unknownRouteHandler(event, context, requestPath, httpMethod))
            }
            ...
            body = result;
            console.log(config, "Route handler success", { body: body });
        } catch (error) {
            if (!error.statusCode) {
                // log unrecognised error
                console.log(config, "Route handler failure", { error: error });
